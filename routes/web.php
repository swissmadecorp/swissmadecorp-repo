...
Route::group(['middleware' => ['web','throttle:60,1',BlockIpMiddleware::class]], function () {
    ...
    Route::get('thankyou', function () {
        ...
        if (is_array($order)) {
            ...
            $order = $orderId
                ? \App\Models\Order::with(['products.images'])->find($orderId)
                : null;
        } elseif ($order instanceof \App\Models\Order) {
            $order->loadMissing(['products.images']);
        } else {
            $order = null;
        }

        return view('payment/thankyouorder', ['order' => $order]);
