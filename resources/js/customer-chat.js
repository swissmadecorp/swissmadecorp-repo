...
function initCustomerWidget(root) {
    ...
    function scheduleTypingIdleReset() {
        if (typingIdleTimer) {
            window.clearTimeout(typingIdleTimer);
        }
    ...
    function announceCustomerTypingActivity() {
        if (!activeToken || shouldUseConversationEmailFallback()) {
            return;
        }
    ...
    messageInput.addEventListener('input', () => {
        ...
        announceCustomerTypingActivity();
    });

    messageInput.addEventListener('keydown', (event) => {
        if (!activeToken || event.ctrlKey || event.metaKey || event.altKey) {
            return;
        }
...
function initStaffWidget(root) {
    ...
    let typingIdleTimer = null;
    let lastTypingState = false;
    let customerTypingResetTimer = null;
    ...
    function clearCustomerTypingResetTimer() {
        if (customerTypingResetTimer) {
            window.clearTimeout(customerTypingResetTimer);
            customerTypingResetTimer = null;
        }
    }

    function holdCustomerTypingIndicator(chatId) {
        clearCustomerTypingResetTimer();

        customerTypingResetTimer = window.setTimeout(() => {
    ...
    async function loadChat(chatId, options = {}) {
        ...
        try {
            ...
            activeTyping = data.typing ?? activeTyping;
            if (activeTyping?.customer?.is_typing) {
                holdCustomerTypingIndicator(data.chat.id);
            } else {
                clearCustomerTypingResetTimer();
            }
    ...
    async function handleStaffEvent(payload) {
        ...
        if (payload.typing && activeChat?.id != null && Number(activeChat.id) === Number(payload.chat?.id)) {
            activeTyping = payload.typing;
            if (payload.typing?.customer?.is_typing) {
                holdCustomerTypingIndicator(payload.chat.id);
            } else {
                clearCustomerTypingResetTimer();
            }
        }
