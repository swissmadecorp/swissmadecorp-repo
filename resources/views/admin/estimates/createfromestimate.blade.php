...
<form method="POST" action="{{route('orders.store', $estimate->id)}}" id="estimateFrom">
    ...
    <?php $totalRows=0; ?>
        ...
        ?>
        <tr>
        <td><input value="1" class="previously_loaded" type="hidden" />
            <input style="width: 65px" value="{{$product->id}}" class="form-control product_id number" autofocus autocomplete="off" type="text" pattern="\d*" name="id[]"></td>
        <td><?=$path ?></td>
        <td>
            <input class="form-control" class="product_name" value="{{ !$product->pivot->product_name ? $product->title : $product->pivot->product_name}}" name="product_name[]" type="text">
        ...
        <td>
            <div class="col-2 input-group">
                ...
                <input style="width: 80px" class="form-control pricecalc" pattern="^\d*(\.\d{0,2})?$" name="price[]" value="{{$product->pivot->price}}" type="text"></td>
            </div>
...
<b>Freight: &nbsp;</b>
<input name='freight' id="s_freight-input" style="width: 100px; text-align: right;display:inline" class="form-control" value="{{$estimate->freight}}" />
</div>
...
<script>
    ...
    $(document).ready( function() {
        ...
        $(document).on('blur', '.product_id', function(e) {
            ...
            if ($(this).val() != '') {
                _this = $(this);
                if ($(_this).prev().attr("class") == "previously_loaded") return true
                var is_onmemo = '';
