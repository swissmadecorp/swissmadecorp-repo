...
<table id="repairs" class="table table-striped table-bordered hover" cellspacing="0" width="100%">
    <thead>
        <tr>
            <th></th>
            <th>Id</th>
    ...
    @foreach ($repairs as $repair)
        <tr>
            ...
            <td>{{ $repair->status == 1 ? "Complete" : "Incomplete" }}</td>
            <td>{{ $repair->created_at->format('m-d-Y')}}</td>
            <td>{{ $repair->updated_at->format('m-d-Y')}}</td>
        </tr>
    @endforeach
    </tbody>
</table>
</div>
<form action="repairs/create">
...
<script>
    ...
    $(document).ready( function() {
        ...
        $('#repairs tbody').on('click', 'td', function () {
            ...
            if (table.api().column( this ).index() != 0) {
                ...
                window.location.href = 'repairs/'+data[1]+'/edit';
            }
        } );
        ...
        $('.dropdown-menu button').click( function(e) {
            ...
            if (confirm('Are you sure you want to delete selected product?')) {
                window.location.href="repairs/"+id[0][1]+'/destroy';
            }
        })
    })    
</script>
