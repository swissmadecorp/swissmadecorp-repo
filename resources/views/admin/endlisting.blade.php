...
<a href="sync" class="btn btn-primary sync">Synchronize</a>
<hr>
...
<script>
    ...
    $(document).ready( function() {
        var table = $('#products').DataTable({
            ...
            ajax: {
                url: "{{ URL::route('loadItems','_token='.csrf_token()) }}",
        ...
        $(document).on('click','.editlisting', function (e) {
            e.preventDefault();
            itemId = $(this).parent('td').next().children().first().text();
            eBayitemId = $(this).parent('td').children().first().text()
            location.href = '/admin/ebay/'+itemId+'/edit/'+eBayitemId;
        })
        ...
        $('.sync').click( function(e) {
            ...
            $.each (data, function(key,value) {
                ...
                itemId=$(data[key][1]).text().replace(/\s/g,'');

                _ids.push([id,itemId,data[key][5]]);
            })
            
            var data = {
                ids: _ids
            };
            $.post("{{route('ebay.syncronize')}}", data, function(response) {
                $.alert('done');
            });
        ...
        $( "#reasonDialog" ).dialog({
			buttons: {
				"End Listing": function() {		
					var data = {
						_token: csrf_token,
					   datainfo: $('#formReason').serialize()
					};
					$.post("{{route('ebay.end.item')}}", data, function(response) {
                        if (response.error==0) {
                            $(endItemLink).parent().prev().prev().text('inactive')
                            $(endItemLink).hide()
                            $(endItemLink).next().hide()
