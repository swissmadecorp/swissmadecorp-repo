        ...
        <div class="grid gap-6 transition-all duration-700 ease-in-out items-start"
            ...
            <div x-show="active === null || active === 2"
                ...
                <div class="flex flex-col h-full justify-between">
                    ...
                    <button x-show="active === 2" @click.stop="active = null" x-cloak class="self-start text-sm bg-black/40 p-2 px-6 rounded-full text-white">✕ Close</button>
                </div>
            </div>
            ...
            <div x-show="active === null || active === 3"
                ...
                <div class="flex flex-col h-full justify-between">
                    ...
                    <button x-show="active === 3" @click.stop="active = null" x-cloak class="self-start text-sm bg-black/40 p-2 px-6 rounded-full text-white">✕ Close</button>
                </div>
            </div>
        </div>
                            ...
                            <template x-for="user in $wire.users" :key="user.id">
                                <tr class="border-b hover:bg-white/50 transition-colors">
                                    ...
                                    <td class="px-3 py-2">
                                        ...
                                        <div x-show="editingId === user.id" x-cloak>
                                            <input x-model="user.name" class=" bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" :class="{'input-error': $wire.errors.name}">
                                            <template x-if="$wire.errors.name">
                                                <span class="text-[10px] text-red-500 font-semibold block" x-text="$wire.errors.name"></span>
                                            </template>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-3 py-2">
                                        ...
                                        <div x-show="editingId === user.id" x-cloak>
                                            <input x-model="user.username" class=" bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" :class="{'input-error': $wire.errors.username}">
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-3 py-2 whitespace-nowrap">
                                        <div x-show="editingId !== user.id" class="flex gap-3">
                                            <button @click="editingId = user.id; $wire.resetUIErrors()" class="text-blue-600 hover:underline">Edit</button>
                                        ...
                                        <div x-show="editingId === user.id" class="flex gap-3" x-cloak>
                                            <button @click="$wire.saveUser({ ...user }).then(() => { editingId = null; $wire.resetUIErrors(); }).catch(() => {})"
                                                    class="text-green-600 font-bold hover:underline">Save</button>
                                            <button @click="editingId = null; $wire.resetUIErrors(); $wire.loadData()"
                            ...
                            <template x-for="role in $wire.roles" :key="role.id">
                                <tr class="border-b">
                                    ...
                                    <td class="px-3 py-2">
                                        ...
                                        <div x-show="editingId === role.id" x-cloak>
                                    ...
                                    <td class="px-3 py-2 text-xs text-gray-500 italic" x-text="role.permissions_list"></td>
                                    <td class="px-3 py-2">
                                        <div x-show="editingId !== role.id" class="flex gap-3">
                                            <button @click="editingId = role.id; $wire.resetUIErrors()" class="text-blue-600 hover:underline">Edit</button>
                                        ...
                                        <div x-show="editingId === role.id" class="flex gap-3" x-cloak>
                                            <button @click="$wire.saveRole({ ...role }).then(() => { editingId = null; }).catch(() => {})"
                                                    class="text-green-600 font-bold hover:underline">Save</button>
                            ...
                            <template x-for="perm in $wire.permissions" :key="perm.id">
                                <tr class="border-b">
                                    ...
                                    <td class="px-3 py-2">
                                        ...
                                        <div x-show="editingId === perm.id" x-cloak>
                                    ...
                                    <td class="px-3 py-2">
                                        <div x-show="editingId !== perm.id" class="flex gap-3">
                                            <button @click="editingId = perm.id; $wire.resetUIErrors()" class="text-blue-600 hover:underline">Edit</button>
                                        ...
                                        <div x-show="editingId === perm.id" class="flex gap-3" x-cloak>
                                            <button @click="$wire.savePermission({ ...perm }).then(() => { editingId = null; }).catch(() => {})"
                                                    class="text-green-600 font-bold hover:underline">Save</button>
                                            <button @click="editingId = null; $wire.loadData()" class="text-gray-400 hover:underline">Cancel</button>
                                        </div>
                                    </td>
