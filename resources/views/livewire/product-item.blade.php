            ...
            <div id="default-styled-tab-content" class="dark:bg-gray-800">
                <div class="p-4 rounded-lg dark:bg-gray-800" id="product" role="tabpanel" aria-labelledby="product-tab" x-bind:class="{ 'hidden': activeTab !== 'product' }">
                    <form>
                        ...
                        <div class="grid gap-2 md:grid-cols-4 mt-2">
                            <div>
...
@script
    <script>
        $(function() {
            ...
            let productFormRequest = Promise.resolve();
            let productFormOpening = false;

            function queueProductFormRequest(callback) {
                productFormRequest = productFormRequest
                    .catch(() => undefined)
                    .then(callback);
            ...
            function labelForField(element) {
                ...
                if ($(element).closest('.image-container').length && element.type !== 'file') {
                    return null;
                }
            ...
            $(document).on('click', '.editproduct', function() {
                if ($('#slideover-product-container').hasClass('invisible')==true && !productFormOpening) {
                    productFormOpening = true;
                    const productId = $(this).attr('data-id');
                    const isDuplicate = !!$(this).attr('data-duplicate');
                    const duplicateValue = isDuplicate ? Number($(this).attr('data-duplicate')) : 0;

                    Slider()

                    queueProductFormRequest(() => $wire.$call('openItem', Number(productId), duplicateValue))
                        .then(() => {
                            if ($('#slideover-product-container').hasClass('invisible')) {
                                return;
                            }
                        ...
                        .catch(error => console.error('Unable to open product form.', error))
                        .finally(() => {
            ...
            window.closeFields = function() {
                ...
                Slider()
                queueProductFormRequest(() => $wire.$call('closeItem'))
                    .catch(error => console.error('Unable to clear product form.', error));
            }
            ...
            $('.newproduct').click(function() {
                ...
                queueProductFormRequest(() => $wire.$call('beginCreate', groupId))
                    .then(() => {
                        if ($('#slideover-product-container').hasClass('invisible')) {
                            return;
                        }

                        startTrackingHeartbeat('create')
                    })
                    .catch(error => console.error('Unable to open new product form.', error))
                    .finally(() => {
                        productFormOpening = false;
                    });
            })
