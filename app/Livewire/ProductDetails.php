<?php

namespace App\Livewire;

use Livewire\Attributes\Layout;
use Illuminate\Support\Facades\Request;
use Livewire\Component;
use Livewire\Attributes\On;
use App\Models\DiscountRule;
use App\Models\Category;
use App\Models\Product;
use App\Models\Cart;

class ProductDetails extends Component
{
    private const BREADCRUMB_FILTERS = ['brand', 'model', 'condition', 'gender', 'casesize'];

    public $slug;
    public $breadcrumbs = [];
    // public $totalcart;
    public $discount;
    public $product;
    public $custom_columns;
    public $mainImage;
    public $allImages;
    public $productStatus = [];
    public $p_status;

    public $search = "";

    private function discountRule() {
        $now = (date('Y-m-d',strtotime(now())));
        $discountRule = DiscountRule::whereIn('action',[4,5])
            ->where('start_date','<=',$now)
            ->where('end_date','>=',$now)
            ->where('is_active', '1')
            ->first();

        return $discountRule;
    }

    private function normalizeBreadcrumbValue($value)
    {
        return is_string($value) ? trim($value) : $value;
    }

    private function buildBreadcrumbs(Product $product): array
    {
        return array_filter([
            'brand' => $this->normalizeBreadcrumbValue($product->categories?->category_name),
            'model' => $this->normalizeBreadcrumbValue($product->p_model),
            'condition' => $this->normalizeBreadcrumbValue(Conditions()->get($product->p_condition)),
            'gender' => $this->normalizeBreadcrumbValue($product->p_gender),
            'casesize' => $this->normalizeBreadcrumbValue($product->p_casesize),
        ], fn ($value) => filled($value));
    }

    private function breadcrumbParamsFor(string $key): array
    {
        $filters = [
            'search' => $this->normalizeBreadcrumbValue($this->search),
            'brand' => '',
            'model' => null,
            'condition' => '',
            'gender' => '',
            'casesize' => '',
        ];

        foreach (self::BREADCRUMB_FILTERS as $filter) {
            if (!array_key_exists($filter, $this->breadcrumbs)) {
                continue;
            }

            $filters[$filter] = $this->normalizeBreadcrumbValue($this->breadcrumbs[$filter]);

            if ($filter === $key) {
                break;
            }
        }

        return array_filter($filters, fn ($value) => filled($value));
    }

    public function setBread($key) {
        $breadcrumbs = $this->breadcrumbs;

        if (!array_key_exists($key, $breadcrumbs)) {
            return redirect()->route('watch.products');
        }

        return redirect()->route('watch.products', $this->breadcrumbParamsFor($key));
    }

    #[On('delete-from-cart-message')]
    public function dispatchRefresh($id) {
        $this->productStatus[$id] = 0;
        // $this->dispatch('refresh-cart-count');
    }

    public function BuyNow($id) {
        $this->dispatch('add-to-cart',$id,'buynow');
        $this->productStatus[$id] = 2;
    }

    public function AddToCart($id) {
        $this->productStatus[$id] = 2;
        $this->dispatch('add-to-cart',$id);
        $this->dispatch('refresh-cart-count');
    }

    public function mount()
    {
        $this->slug = Request::route('slug');
        $this->search = $this->normalizeBreadcrumbValue(Request::query('search', ''));

        $custom_columns = getCustomColumns();

        $product = Product::with('images','categories')->where('slug',$this->slug)
            ->whereNotIn('p_status',[4,7,9])
            ->first();

        $this->product = $product;
        if ($product) {
            $this->breadcrumbs = $this->buildBreadcrumbs($product);

            // $this->totalcart = Cart::products();
            $this->discount = DiscountRule::currentPricingRuleForProduct($product);
            // $this->breadcrumbs = $this->breadcrumbs;

            $imageMain=$product->images()->first();
            if ($imageMain) {
                $this->mainImage = "/images/$imageMain->location";
                $this->allImage = $product->images->pluck('location')->map(fn($location) => '/images/' . $location);
            } else {
                $this->mainImage = 'images/no-image.jpg';;
            }
        }

        // You can also load product details based on the slug here
        // $this->product = Product::where('slug', $slug)->first();
    }

    public function setMainImage($imagePath) {
        $this->mainImage = "/images/$imagePath";
    }

    #[Layout('components.layouts.app')]
    public function render() {
        if ($this->product)
            $this->p_status = array_key_exists($this->product->id, $this->productStatus) ? $this->productStatus[$this->product->id] : 0;

        return view('livewire.product-details', ['breadcrumbs' => $this->breadcrumbs])
            ->layoutData(['pageName' => 'Product Details'])
            ->title("Product Details");
    }
}
