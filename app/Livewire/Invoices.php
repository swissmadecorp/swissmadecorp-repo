...
class Invoices extends Component
{
    ...
    public function loadInvoice($id) {
        ...
        // Opening the invoice slide does not change the invoice list. Avoid
        // recounting and re-rendering thousands of paid invoices just to relay
        // the load event to the InvoiceItem component.
        $this->skipRender();
    }
        ...
        if ($this->search == "*") {
            ...
            $orderQuery = Order::with(['customers', 'payments', 'products'])
                ->whereHas('customers', function($query) use($choices) {
                    $query->whereIn('customers.cgroup', $choices);
                })
                ->where('status', $status)
